...
$title = $director = $year = $runtime = $description = $trailer = $poster = $actor1 = $actor2 = $actor3 = $genre1 = $genre2 = $genre3 = "";
...
//    $poster = filter_var($poster, FILTER_SANITIZE_URL);
    ...
    if(empty($title_err) && empty($director_err) && empty($description_err) && empty($year_err) && empty($runtime_err) && empty($trailer_err) && empty($poster_err) && empty($actor1_err) && empty($actor2_err) && empty($actor3_err)){
        ...
        if(($stmt = mysqli_prepare($link, $sql)) && ($stmt_d = mysqli_prepare($link, $sql_d)) && ($stmt_a1 = mysqli_prepare($link, $sql_a1)) && ($stmt_a2 = mysqli_prepare($link, $sql_a2)) && ($stmt_a3 = mysqli_prepare($link, $sql_a3))){
            ...
            // Set parameters
            $param_title = addslashes(trim($_POST["title"]));
            ...
            // Set parameters
            $param_title_d = addslashes(trim($_POST["title"]));
            $param_name_d = addslashes(trim($_POST["director"]));
            $param_role_d = "director";
            // Set parameters
            $param_title_a1 = addslashes(trim($_POST["title"]));
            $param_name_a1 = addslashes(trim($_POST["actor1"]));
            $param_role_a1 = "actor";
            // Set parameters
            $param_title_a2 = addslashes(trim($_POST["title"]));
            $param_name_a2 = addslashes(trim($_POST["actor2"]));
            $param_role_a2 = "actor";
            // Set parameters
            $param_title_a3 = addslashes(trim($_POST["title"]));
            $param_name_a3 = addslashes(trim($_POST["actor3"]));
            $param_role_a3 = "actor";
...
<div class="grid-container-main">
    ...
    <p class="section-title">Add a New Movie</p>
        ...
        <table style="width: 100%; align-content: center">
            <tr>
                ...
                <td style="float: right">
                    <button class="myButton myButtonPrimary" type="submit">Add Movie</button>
                </td>
            </tr>
        </table>
        </form>
